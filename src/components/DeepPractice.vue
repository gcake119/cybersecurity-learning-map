...
load();watch(key,load);
watch([round,showNotes],async()=>{await nextTick();document.querySelector('.lesson-stages')?.scrollIntoView({block:'start'});});
...
<section ref="output" class="deep-output" aria-label="深入實驗結果"><div v-if="!result" class="lab-empty"><h3>先預測，再觀察</h3><p>配置條件並送出預測後，動畫會在關鍵檢查點暫停。確認此時已知與未知的資訊，再繼續播放。</p></div><template v-else><p v-if="stale" class="stale">條件或已知證據已改變，下方仍是上一次的結果。</p><div class="prediction-feedback" :class="{correct:result.correct}"><strong>{{result.correct?'預測與結果一致':'結果和你的預測不同'}}</strong><p>你預測：{{lab.predictions.find(p=>p[0]===result!.prediction)?.[1]}}</p><p>{{result.accepted?'✓ 這次證據符合本輪目標。':'這次尚未符合本輪目標；請根據結果重新配置與預測。'}}</p></div><ExperimentAnimation :id="mission?8:id" :current="result" :previous="previous" :controls="lab.controls" generic :checkpoint="lab.checkpoint" :checkpoint-at="!mission&&[2,4,5].includes(id)?2:1"/><p class="lab-explanation">{{result.outcome.lesson}}</p></template></section></div>
