...
import SystemWorkbench from './SystemWorkbench.vue';
import {definitions,scenarios} from '../labs/workbench';
...
const stage=ref(0),legacy=ref(false);
const nextUnit=computed(()=>units[units.findIndex(u=>u.id===id.value)+1]);
...
watch(id,()=>{reset();stage.value=0;legacy.value=false;answers.value=[-1,-1];submitted.value=false;loadNote();});function key(e:KeyboardEvent){if(e.key==='Escape')router.push(`/map/${id.value}`);}
...
<section v-if="stage===0" class="lesson-intro"><p class="core-conclusion">{{unit.conclusion}}</p><h2>學完這個單元，你應該能</h2><ul><li v-for="goal in unit.goals" :key="goal">{{goal}}</li></ul><CourseDiagram :id="id"/><div class="concept-lessons"><article v-for="([title,body],i) in unit.concepts" :key="title"><span>觀念 {{i+1}}</span><h3>{{title}}</h3><p>{{body}}</p></article></div><section class="scenario-brief"><h2>這次的情境</h2><p>{{scenarios[id]}}</p><h3>操作路線</h3><ol><li v-for="step in definitions[id]!.tasks" :key="step">{{step}}</li></ol></section><div class="lesson-actions"><button class="primary" @click="stage=1">開始情境操作 →</button></div></section>
<template v-if="stage===1">
<div class="lab-tool-switch"><button :aria-pressed="!legacy" @click="legacy=false">系統互動實驗室</button><button :aria-pressed="legacy" @click="legacy=true">基礎請求工具</button></div>
<SystemWorkbench v-if="!legacy" :id="id" @complete="met=true"/>
<div v-if="!legacy" class="lesson-actions"><button class="primary" @click="stage=0">← 回到觀念</button><button class="primary" @click="stage=2">解釋結果與應用 →</button></div>
