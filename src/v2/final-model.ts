...
export function evaluateFinal(c:Record<string,Value>,step:number):Result {
  ...
  node('browser','ok',`外部協作者請求 ${c.asset}`);
  ...
  node('storage',workerRead?'bad':previewChanged?'warn':'ok',workerRead?`失守 Worker 能使用 ${c.workerScope} scope`:previewChanged?'預覽資料可改變 mock 畫面結構':'固定文字／允許結構的預覽');
  ...
  edge('browser','api','document ID／角色',c.isolate?'blocked':requestExposed?'bad':'ok');edge('api','db','讀取前的物件政策',requestExposed?'bad':'blocked');edge('api','worker','背景預覽入口','warn');edge('worker','db','identity ＋ route ＋ scope',workerRead?'bad':'blocked');edge('worker','storage','指定物件能力',workerRead?'bad':'blocked');edge('worker','provider','獨立／共用 credential',providerUse?'bad':'blocked');edge('storage','browser','预览解讀',previewChanged?'warn':'ok');edge('repo','ci','dependency／build',versionRisk?'warn':'ok');edge('ci','artifact','產物發布',c.ciScope==='write'?'warn':'blocked');edge('artifact','deploy','核對來源',c.provenance?'ok':'unknown');edge('deploy','app','部署能力',deploymentActive?'bad':'ok');edge('app','db','現有 runtime 資料能力',appRead?'bad':'blocked');edge('app','logs','事件欄位',r.nodes.logs);edge('logs','alerts','規則涵蓋',r.nodes.alerts);
