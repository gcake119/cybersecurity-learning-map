...
router.beforeEach(to => { if(String(to.path).startsWith('/v2')) return true; const max=to.name==='mission'?2:8; const index=Number(to.params.index); if(index>max)return {name:to.name!,params:{index:max},replace:true}; });
